        ...
        stage('Dockerfile lint') {
            steps {
                script {
                    ...
                    '''
                    if (sh(label: 'Running dockerfile lint with hadolint', script: 'hadolint docker/gp2gp_translator/Dockerfile', returnStatus: true) != 0) {error("Dockerfile linting failed")}
                }
            }
        }
        stage('Build') {
