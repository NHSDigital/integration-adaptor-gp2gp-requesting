...
past, historic and current records) it is best practice for all the records to be checked in advance of being shared.
...
1. `RequestStatement / confidentialityCode`.
2. `EhrComposition / confidentialityCode`.
...
2. Any contained `ObservationStatement / confidentialityCode` within a `CompoundStatement` which is a `Battery` has the `NOPAT` security label applied.
